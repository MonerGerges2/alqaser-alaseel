            ...
            <ul className="text-sm space-y-2">
              {[
                ...
                <li key={index} className="group transition duration-300 hover:text-gold">
                  {item.to.startsWith("/") ? (
                    <Link to={item.to} className="flex items-center justify-center gap-2 md:justify-start">
                      {item.label}
                      <GoArrowUpLeft className="ml-2 text-sm transition-transform duration-300 group-hover:-rotate-45" />
                    </Link>
                  ) : (
                    <HashLink to={item.to} className="flex items-center justify-center gap-2 md:justify-start">
                      {item.label}
                      <GoArrowUpLeft className="ml-2 text-sm transition-transform duration-300 group-hover:-rotate-45" />
                    </HashLink>
                  )}
